...
public class ChessController implements LocationListener, ChessMenuController {
    ...
    private Move makeMove(Piece piece, Location location) {
        ...
        return chess.getAllowedMoves().stream().filter(m -> m.piece.equals(piece) && m.getEndpointLocation().equals(location)).findFirst().orElse(null);
    }
    ...
    private void draw() {
        ...
        if (currentSelectedPiece != null) {
            ...
            this.chess.getAllowedMoves().stream().filter(m -> m.piece.equals(currentSelectedPiece)).forEach(m -> {
    ...
    public boolean isLocationDraggable(Location location) {
        return !finished && chess.getBoard().isOccupied(location) &&
               chess.getBoard().getPiece(location).color.equals(chess.getTurnPlayer().getColor());
    }

    private void checkForFinish() {
        switch (chess.getStatus()) {
            case CHECKMATE -> JOptionPane.showMessageDialog(boardPanel, "Checkmate! " + chess.getTurnPlayer().getColor().opposite() + " wins!");
